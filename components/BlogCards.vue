<template>
    <div class="flex flex-col justify-start items-start" @mouseenter="mouseenter" @mouseleave="mouseleave">
        <div class="relative">  
            <img :src="src" :alt="alt" class="rounded-md aspect-square xl:aspect-auto object-cover">
            <div class=" absolute h-full w-full bg-gradient-to-b from-[rgba(13,13,13,0.5)] via-transparent to-[rgba(13,13,13,0.5)] top-0 left-0 rounded-md opacity-1 lg:opacity-0" ref="overlay"></div>
        ...
        <div class="flex flex-col justify-start items-start gap-2 mt-3 px-2">
                <AnimsAnimButton :scaleOverride="true" :action="action" :duration="1" :animate="animate" cursorType="cursor-pointer"
                    class=" border-[1px] bg-light text-dark border-solid border-light px-3 text-sm py-1 rounded-md">
                    {{ category }}
                </AnimsAnimButton>
            <p class="text-lg font-normal w-full"><slot></slot></p>
        </div>
    </div>
</template>
